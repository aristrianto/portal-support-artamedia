...
export default function DocumentCategoryPage({ moduleKey, category, scope, title, description, breadcrumb }) {
  ...
  const exportCsv = () => {
    const headers = ['Judul','No Dokumen','Tgl Dokumen','Berlaku Dari','Kedaluwarsa','Pelanggan','Mitra','Status','Deskripsi'];
    const rows = items.map(i => [i.title,i.doc_number,i.doc_date,i.valid_from,i.valid_until,cmap[i.customer_id]||'-',pmap[i.partner_id]||'-',i.status,i.description].map(v => `"${(v ?? '').toString().replace(/"/g,'""')}"`).join(','));
              ...
              <TableBody>
                ...
                {!loading && items.map((it) => (
                  <TableRow key={it.id} data-testid={CRUD.row(moduleKey, it.id)} className="hover:bg-accent/40">
                    ...
                    <TableCell className="text-sm">{pmap[it.partner_id] || '-'}</TableCell>
          ...
          {previewDoc && (
            <div className="space-y-3 text-sm">
              <div className="grid grid-cols-2 gap-3">
                ...
                <I k="Pelanggan" v={cmap[previewDoc.customer_id] || '-'} />
                <I k="Mitra" v={pmap[previewDoc.partner_id] || '-'} />
              </div>
