...
Role = Literal['admin', 'supervisor', 'engineer', 'viewer']
...
class InterconnectionIn(BaseModel):
    ...
    dest_rack: str = ""
    dest_rack_id: Optional[str] = None  # kept for backward compatibility with older records
    dest_device: str = ""
    dest_port: str = ""
...
BroadbandTicketStatus = Literal['Open', 'Progress', 'Monitoring', 'Resolved', 'Closed']
DedicatedTicketStatus = Literal['Open', 'Investigation', 'Provider Coordination', 'Monitoring', 'Resolved', 'Closed']


class BroadbandTicketIn(BaseModel):
    model_config = ConfigDict(extra="allow")
    ticket_number: str
    customer_id: Optional[str] = None
    cid: str = ""
    ...
    status: BroadbandTicketStatus = 'Open'
    engineer: str = ""
...
class DedicatedTicketIn(BaseModel):
    model_config = ConfigDict(extra="allow")
    ticket_number: str
    customer_id: Optional[str] = None
    sid: str = ""
    ...
    internal_pic: str = ""
    provider_pic: str = ""
    status: DedicatedTicketStatus = 'Open'
    open_time: Optional[str] = None
...
build_crud("shift-handovers", "shift_handovers", ShiftHandoverIn, ["officer", "site", "issue", "action_taken"])
...
async def get_counts(user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "crm": {
            "broadband": await c("broadband_tickets"),
            "broadband_open": await c("broadband_tickets", {"status": {"$nin": ["Closed", "Resolved"]}}),
            "dedicated": await c("dedicated_tickets"),
            "dedicated_open": await c("dedicated_tickets", {"status": {"$nin": ["Closed", "Resolved"]}}),
        },
    }
...
async def crm_stats(user: dict = Depends(get_current_user)):
    async def agg_status(coll, allowed):
        result = {s: 0 for s in allowed}
        async for d in db[coll].find({}, {"_id": 0, "status": 1}):
            s = d.get("status")
            if s in result:
                result[s] += 1
        return result

    async def agg_field(coll, field):
        result = {}
    ...
    bb_status = await agg_status("broadband_tickets", ['Open', 'Progress', 'Monitoring', 'Resolved', 'Closed'])
    dd_status = await agg_status("dedicated_tickets", ['Open', 'Investigation', 'Provider Coordination', 'Monitoring', 'Resolved', 'Closed'])

    bb_prio = await agg_field("broadband_tickets", "priority")
    dd_prio = await agg_field("dedicated_tickets", "priority")

    # Provider (partner_id -> name)
    partners_map = {}
    async for p in db.partners.find({}, {"_id": 0, "id": 1, "name": 1}):
        partners_map[p["id"]] = p.get("name") or "-"

    async def by_provider(coll):
        r = {}
        async for d in db[coll].find({}, {"_id": 0, "partner_id": 1}):
            name = partners_map.get(d.get("partner_id"), "Tanpa Provider")
            r[name] = r.get(name, 0) + 1
        return r

    bb_prov = await by_provider("broadband_tickets")
    dd_prov = await by_provider("dedicated_tickets")

    now = datetime.now(timezone.utc)

    # SLA breach: has sla_target_minutes and not closed and (now - open_time) > sla_target
    async def over_sla(coll):
        cnt = 0
        async for d in db[coll].find({"status": {"$nin": ["Closed", "Resolved"]}, "sla_target_minutes": {"$gt": 0}}, {"_id": 0, "open_time": 1, "sla_target_minutes": 1}):
            try:
                ot = d.get("open_time")
                if not ot:
                    continue
                # accept 'Z' suffix
                open_dt = datetime.fromisoformat(ot.replace("Z", "+00:00"))
                if (now - open_dt).total_seconds() / 60 > (d.get("sla_target_minutes") or 0):
                    cnt += 1
            except Exception:
                pass
        return cnt

    # Trend last 14 days by open_time
    trend = []
    for i in range(13, -1, -1):
        d0 = now - timedelta(days=i)
        day_key = d0.strftime("%Y-%m-%d")
        bb = await db.broadband_tickets.count_documents({"open_time": {"$regex": f"^{day_key}"}})
        dd = await db.dedicated_tickets.count_documents({"open_time": {"$regex": f"^{day_key}"}})
        trend.append({"date": day_key, "broadband": bb, "dedicated": dd})

    total_open = (
        await db.broadband_tickets.count_documents({"status": "Open"}) +
        await db.dedicated_tickets.count_documents({"status": "Open"})
    )
    in_progress = (
        await db.broadband_tickets.count_documents({"status": {"$in": ["Progress", "Monitoring"]}}) +
        await db.dedicated_tickets.count_documents({"status": {"$in": ["Investigation", "Provider Coordination", "Monitoring"]}})
    )
    closed = (
        await db.broadband_tickets.count_documents({"status": {"$in": ["Closed", "Resolved"]}}) +
        await db.dedicated_tickets.count_documents({"status": {"$in": ["Closed", "Resolved"]}})
    )
    over_sla_total = (await over_sla("broadband_tickets")) + (await over_sla("dedicated_tickets"))

    total_tickets = (
        await db.broadband_tickets.count_documents({}) +
        await db.dedicated_tickets.count_documents({})
    )

    # Merge providers
    all_providers = {}
    for k, v in bb_prov.items():
        all_providers[k] = all_providers.get(k, 0) + v
    for k, v in dd_prov.items():
        all_providers[k] = all_providers.get(k, 0) + v

    # Merge priorities
    all_prio = {}
    for k, v in bb_prio.items():
        all_prio[k] = all_prio.get(k, 0) + v
    for k, v in dd_prio.items():
        all_prio[k] = all_prio.get(k, 0) + v
    ...
    return {
        ...
        "closed": closed,
        "over_sla": over_sla_total,
        "broadband_by_status": bb_status,
        "dedicated_by_status": dd_status,
        "by_provider": all_providers,
        "by_priority": all_prio,
        "trend": trend,
    }
...
async def on_startup():
    ...
    await db.broadband_tickets.create_index("ticket_number")
    await db.broadband_tickets.create_index("status")
    await db.dedicated_tickets.create_index("ticket_number")
    await db.dedicated_tickets.create_index("status")
    ...
    # Expose db + start auto-sync background loop
    app.state.db = db
